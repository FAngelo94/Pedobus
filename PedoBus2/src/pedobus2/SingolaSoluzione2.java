 ...
 * @author Angelo
 */
public class SingolaSoluzione2 {
    Ottimizza o;
    double archi[][];
    int indexArchi;
    int rami;
    int usati[];
    int prof;
    public SingolaSoluzione2(Ottimizza o, double archiM[][], int indexArchiM, int ramiM, int usatiM[], int profM){
        this.o=o;
        this.rami=ramiM;
        ...
        if(nodi!=null)
        {
            ...
            prof++;
            int nodoPrec=0;
            while(nodi!=null && check(archiM.length,j,prof))//(j<=2 && prof<0 || j<2)
            {
    ...
    public boolean check(int l, int j, int prof)
    {
        switch(l)
        {
            case 10:
            ...
            case 20:
                ...
                break;
            case 30:
                if(j<2 && prof<14 || j<3 && prof<6 || j<4 && prof<3 || j<1)
                    return true;
                break;
            case 50:
                if(j<2 && prof<13 || j<3 && prof<5 || j<4 && prof<2 || j<1)
                    return true;
                break;
            case 80:
                if(j<2 && prof<13 || j<3 && prof<4 || j<1)
                    return true;
                break;
            case 100:
                if(j<2 && prof<14 || j<3 && prof<3 || j<1)
                    return true;
                break;
            case 150:
                if(j<2 && prof<13 || j<3 && prof<1 || j<3 && prof<1 || j<1)
                    return true;
                break;
            case 200:
                if(j<2 && prof<12 || j<3 && prof<1 || j<3 && prof<1 || j<1)
                    return true;
                break;
            case 250:
                if(j<2 && prof<12 || j<3 && prof<1 || j<3 && prof<1 || j<1)
                ...
                break;
            case 300:
                if(j<2 && prof<10 || j<3 && prof<1 || j<3 && prof<1 || j<1)
                    return true;
                break;
        }
        return false;
    ...
    private class ThreadNodiSuccessivi implements Runnable
    {
        ...
        @Override
        public void run() {
            SingolaSoluzione2 SingolaSoluzione=new SingolaSoluzione2(o, archiT, indexArchiT, ramiT, usatiT, prof);
        }
    ...
    private NodiVicini cercaNodiVicini(int nodo)
    {
        ...
        for(int i=0;i<o.n;i++)
        {
            if(usati[i]==0)
            {
                ...
                {
                    n=inserimentoOrdinato(n, i, d+o.distanze[i]);//inserimentoOrdinato(n, i, d+o.distanze[i])
                }
            }
        }
        return n;
